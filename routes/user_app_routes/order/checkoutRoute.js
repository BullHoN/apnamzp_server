...
router.post('/checkout', async (req, res, next) => {
  try {
    ...
    for (let i = 0; i < blockedCODUsers.length; i++) {
      if (blockedCODUsers[i] == order.userId && !order.isPaid) {
        throw createError.BadRequest(
          'You Have Been Blocked By Resturant, Please Try Prepaid Order Or Contact Apna Mzp'
        );
      }
    }
    ...
    if (
      ...
      Number.parseInt(shopData.pricingDetails.minOrderPrice)
    ) {
      throw createError.BadRequest(
        'Item Total Must Be Greater Than ' +
          shopData.pricingDetails.minOrderPrice
      );
    }
